...
from dash_opencellid.utils import (compute_range_created_radio_hist, epsg_4326_to_3857, get_dataset, scheduler_url, )
...
def blank_fig(height):
    ...
    """
    return {"data": [], "layout": {"height": height, "template": template, "xaxis": {"visible": False},
        "yaxis": {"visible": False}, }, }
        ...
        html.Div(
            [
                ...
                children=[
                    ...
                    html.A(
                        html.Img(
                            src="assets/dash-logo.png", style={"float": "right", "height": "50px"},
                            ),
                ...
                children=[
                    ...
                    html.Div(
                        children=[
                            ...
                            dcc.Graph(
                                id="created-histogram", config={"displayModeBar": False}, figure=blank_fig(row_heights[2]),
                                ),
                            html.Button(
                                "Clear Selection", id="clear-created", className="reset-button",
                                ),
                            ],
...
"""),
            ...
            style={"width": "98%", "margin-right": "0", "padding": "10px", },
            className="twelve columns pretty_container", ), ]
    )
...
@app.callback(
    Output("map-graph", "relayoutData"), [Input("reset-map", "n_clicks"), Input("clear-all", "n_clicks")], )
def reset_map(*args):
    return None


@app.callback(
    Output("radio-histogram", "selectedData"), [Input("clear-radio", "n_clicks"), Input("clear-all", "n_clicks")], )
def clear_radio_hist_selections(*args):
    return None


@app.callback(
    Output("range-histogram", "selectedData"), [Input("clear-range", "n_clicks"), Input("clear-all", "n_clicks")], )
def clear_range_hist_selections(*args):
    return None


@app.callback(
    Output("created-histogram", "selectedData"), [Input("clear-created", "n_clicks"), Input("clear-all", "n_clicks")], )
def clear_created_hist_selection(*args):
    return None


@app.callback(
    [Output("indicator-graph", "figure"), Output("map-graph", "figure"), Output("radio-histogram", "figure"),
        Output("range-histogram", "figure"), Output("created-histogram", "figure"), ],
...
def update_plots(
    ...
    if coordinates_4326:
        ...
        coordinates_4326 = [[lon0, lat0], [lon1, lat1], ]
        coordinates_3857 = epsg_4326_to_3857(coordinates_4326)
        # position = {}
        position = {"zoom": relayout_data.get("mapbox.zoom", None),
            "center": relayout_data.get("mapbox.center", None), }
    else:
        position = {"zoom": 0.5, "pitch": 0, "bearing": 0,
            "center": {"lon": data_center_4326[0][0], "lat": data_center_4326[0][1]}, }
        coordinates_3857 = data_3857
        coordinates_4326 = data_4326

    new_coordinates = [[coordinates_4326[0][0], coordinates_4326[1][1]],
        [coordinates_4326[1][0], coordinates_4326[1][1]], [coordinates_4326[1][0], coordinates_4326[0][1]],
    ...
    n_selected_indicator = {
        "data": [{"type": "indicator", "value": n_selected, "number": {"font": {"color": "#263238"}}, }],
        "layout": {"template": template, "height": 150, "margin": {"l": 10, "r": 10, "t": 10, "b": 10}, }, }
    ...
    if n_selected == 0:
        ...
        ddf_small_expr = " & ".join(
            [query_expr_xy] + [f"(radio in {selected_radio_categories})"] + query_expr_range_created_parts
            )
        ddf_small = cell_towers_ddf.query(ddf_small_expr)
        (lat, lon, radio, log10_range, description, mcc, net, created, status,) = dask.compute(
            ddf_small.lat, ddf_small.lon, ddf_small.radio, ddf_small.log10_range, ddf_small.Description, ddf_small.mcc,
        ...
        marker = {"color": radio_codes, "colorscale": radio_colorscale, "cmin": 0, "cmax": 3, "size": 5,
            "opacity": 0.6, }
        ...
        customdata = list(
            zip(
                radio.astype(str), ((10 ** log10_range)).astype(int), [s[:25] for s in to_str_unknown(description)],
                mcc, net, created, to_str_unknown(status), )
            )
        layers = []
    else:
        ...
        layers = [{"sourcetype": "image", "source": img, "coordinates": new_coordinates}]
    ...
    # Build map figure
    map_graph = {"data": [{"type": "scattermapbox", "lat": lat, "lon": lon, "customdata": customdata, "marker": marker,
        "hovertemplate": ("<b>%{customdata[2]}</b><br>"
                          ...
                          "<extra></extra>"), }], "layout": {"template": template, "uirevision": True,
        "mapbox": {"style": "light", "accesstoken": token, "layers": layers, },
        "margin": {"r": 0, "t": 0, "l": 0, "b": 0}, "height": 500, "shapes": [
            {"type": "rect", "xref": "paper", "yref": "paper", "x0": 0, "y0": 0, "x1": 1, "y1": 1,
                "line": {"width": 2, "color": "#B0BEC5", }, }], }, }
    ...
    selected_radio_counts = (agg_range_created_radio.sel(log10_range=range_slice, created=created_slice).sum(
        ["log10_range", "created"]
        ).to_series())
    ...
    return (n_selected_indicator, map_graph, radio_histogram, range_histogram, created_histogram,)
...
def build_radio_histogram(selected_radio_counts, selection_cleared):
    ...
    fig = {"data": [
        {"type": "bar", "x": total_radio_counts.values, "y": total_radio_counts.index, "marker": {"color": bar_bgcolor},
            "orientation": "h", "selectedpoints": selectedpoints,
            "selected": {"marker": {"opacity": 1, "color": bar_bgcolor}},
            "unselected": {"marker": {"opacity": 1, "color": bar_bgcolor}}, "showlegend": False,
            "hovertemplate": hovertemplate, }, ],
        "layout": {"template": template, "barmode": "overlay", "dragmode": "select", "selectdirection": "v",
            "clickmode": "event+select", "selectionrevision": True, "height": 150,
            "margin": {"l": 10, "r": 80, "t": 10, "b": 10},
            "xaxis": {"type": "log", "title": {"text": "Count"}, "range": [-1, np.log10(total_radio_counts.max() * 2)],
                "automargin": True, },
            "yaxis": {"type": "category", "categoryorder": "array", "categoryarray": radio_categories, "side": "left",
                "automargin": True, }, }, }

    # Add selected bars in color
    fig["data"].append(
        {"type": "bar", "x": selected_radio_counts.loc[total_radio_counts.index], "y": total_radio_counts.index,
            "orientation": "h", "marker": {"color": [radio_colors[cat] for cat in total_radio_counts.index]},
            "selectedpoints": selectedpoints, "unselected": {"marker": {"opacity": 0.2}},
...
def build_range_histogram(selected_range_counts, selection_cleared):
    ...
    selectedpoints = False if selection_cleared else None
    hovertemplate = ("count: %{y:,.0}<br>"
                     "range: %{customdata:,} m<br>"
                     "log<sub>10</sub>(range): %{x:.2f}<br>"
                     "<extra></extra>")
    fig = {"data": [{"type": "bar", "x": total_range_counts.index, "y": total_range_counts.values,
        "customdata": (10 ** total_range_counts.index).astype("int"), "marker": {"color": bar_bgcolor},
        "hovertemplate": hovertemplate, "selectedpoints": selectedpoints, "unselected": {"marker": {"opacity": 1.0}},
        "hoverinfo": "y", "showlegend": False, }, ],
        "layout": {"template": template, "barmode": "overlay", "selectionrevision": True, "height": 300,
            "margin": {"l": 10, "r": 10, "t": 10, "b": 10},
            "xaxis": {"automargin": True, "title": {"text": "log<sub>10</sub>(range)"}},
            "yaxis": {"type": "log", "automargin": True, "title": {"text": "Count"}, }, "selectdirection": "h",
            "hovermode": "closest", "dragmode": "select", }, }

    fig["data"].append(
        {"type": "bar", "x": selected_range_counts.index, "y": selected_range_counts.values,
            "customdata": ((10 ** selected_range_counts.index)).astype("int"), "marker": {"color": bar_color},
            "hovertemplate": hovertemplate, "selected": {"marker": {"color": bar_selected_color, "opacity": 1.0}},
            "unselected": {"marker": {"color": bar_unselected_color, "opacity": bar_unselected_opacity, }},
            "selectedpoints": selectedpoints, "hoverinfo": "x+y", "showlegend": False, }
        )
...
def build_created_histogram(selected_created_counts, selection_cleared):
    ...
    fig = {"data": [{"type": "bar", "x": total_created_counts.index, "y": total_created_counts.values,
        "marker": {"color": bar_bgcolor}, "selectedpoints": selectedpoints, "hovertemplate": hovertemplate,
        "unselected": {"marker": {"opacity": 1.0}}, "hoverinfo": "y", "showlegend": False, }, ],
        "layout": {"template": template, "barmode": "overlay", "selectionrevision": True, "height": 300,
            "margin": {"l": 10, "r": 10, "t": 10, "b": 10}, "xaxis": {"title": {"text": "Date"}, "automargin": True},
            "yaxis": {"title": {"text": "Count"}, "type": "log", "automargin": True}, "selectdirection": "h",
            "dragmode": "select", "hovermode": "closest", }, }

    fig["data"].append(
        {"type": "bar", "hoverinfo": "x+y", "x": selected_created_counts.index, "y": selected_created_counts.values,
            "marker": {"color": bar_color}, "selectedpoints": selectedpoints, "hovertemplate": hovertemplate,
